...
import org.ohmcoinj.core.AbstractBlockChain;
import org.ohmcoinj.core.Coin;
import org.ohmcoinj.core.Monetary;
...
public final class MonetaryFormat {
	private static final Logger log = LoggerFactory.getLogger(MonetaryFormat.class);
    ...
    public static final MonetaryFormat UBTC = new MonetaryFormat().shift(6).minDecimals(0).optionalDecimals();
    /** Standard format for fiat amounts. */
    public static final MonetaryFormat FIAT = new MonetaryFormat().shift(0).minDecimals(2).repeatOptionalDecimals(2, 1);
    /** Currency code for base 1 Bitcoin. */
    public static final String CODE_BTC = "MINT";
    /** Currency code for base 1/1000 Bitcoin. */
    public static final String CODE_MBTC = "mMINT";
    /** Currency code for base 1/1000000 Bitcoin. */
    public static final String CODE_UBTC = "µMINT";
    ...
    public CharSequence format(Monetary monetary) {
        ...
        if (decimalGroups != null)
            for (int group : decimalGroups)
                maxDecimals += group;
        if(maxDecimals > monetary.smallestUnitExponent())
        {
        	log.error("size of decimal Groups is {} ",decimalGroups.size());
            if (decimalGroups != null)
                for (int group : decimalGroups)
                    log.error("group value {}", group);
        	log.error("error!!!!!!!!!!!!!!!!!!!!!!!!!maxDecimals {} > smallestUnitExponent {} shift was {} monetary.getValue was {}"
        			,maxDecimals, monetary.smallestUnitExponent(), shift, monetary.getValue());
        }
    ...
    /**
     * Parse a human readable coin value to a {@link org.ohmcoinj.core.Coin} instance.
     * 
    ...
    /**
     * Parse a human readable fiat value to a {@link org.ohmcoinj.core.Fiat} instance.
     * 
     * @throws NumberFormatException
     *             if the string cannot be parsed for some reason
     */
