 ...
 * the lightweight SPV mode, a {@link org.ohmcoinj.store.SPVBlockStore} is the right choice.</p>
 *
 ...
 * {@link org.ohmcoinj.store.H2FullPrunedBlockStore} implements full verification, which is equivalent to the
...
public abstract class AbstractBlockChain {
    ...
    public void addWallet(Wallet wallet) {
        ...
        if (walletHeight != chainHeight) {
            ...
            log.warn("Hashes: {} vs {}", wallet.getLastBlockSeenHash(), getChainHead().getHeader().getScryptHash());
    ...
    public boolean add(FilteredBlock block) throws VerificationException, PrunedException {
        try {
            ...
            throw new VerificationException("Could not verify block " + block.getScryptHash().toString() + "\n" +
                    block.toString(), e);
        }
    }
    ...
    private boolean add(Block block, boolean tryConnecting,
        ...
        try {
            ...
            if (tryConnecting && orphanBlocks.containsKey(block.getScryptHash())) {
                return false;
            }
            ...
            if (shouldVerifyTransactions() && blockStore.get(block.getScryptHash()) != null) {
                return true;
            }
            ...
            if (storedPrev == null) {
                ...
                orphanBlocks.put(block.getScryptHash(), new OrphanBlock(block, filteredTxHashList, filteredTxn));
                return false;
            } else {
                // It connects to somewhere on the chain. Not necessarily the top of the best known chain.
                //checkDifficultyTransitions(storedPrev, block);  As of proof of stake transition blocks change difficulty every block
    ...
    private void connectBlock(final Block block, StoredBlock storedPrev, boolean expensiveChecks,
        ...
        if (!params.passesCheckpoint(storedPrev.getHeight() + 1, block.getScryptHash()))
            ...
            if (haveNewBestChain) {
                ...
                if (splitPoint != null && splitPoint.equals(newBlock)) {
                    ...
                    log.warn("Saw duplicated block in main chain at height {}: {}",
                            newBlock.getHeight(), newBlock.getHeader().getScryptHash());
                    return;
                }
                if (splitPoint == null) {
    ...
    private void tryConnectingOrphans() throws VerificationException, BlockStoreException, PrunedException {
        ...
        do {
            ...
            while (iter.hasNext()) {
                ...
                log.info("Connected orphan {}", orphanBlock.block.getScryptHash());
    ...
    private void checkDifficultyTransitions(StoredBlock storedPrev, Block nextBlock) throws BlockStoreException, VerificationException {
        ...
        StoredBlock cursor = blockStore.get(prev.getScryptHash());
