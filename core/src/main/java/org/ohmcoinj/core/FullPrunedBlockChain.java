...
import org.ohmcoinj.script.Script.VerifyFlag;
...
public class FullPrunedBlockChain extends AbstractBlockChain {    
    ...
    protected StoredBlock addToBlockStore(StoredBlock storedPrev, Block header, TransactionOutputChanges txOutChanges)
        ...
        blockStore.put(newBlock, new StoredUndoableBlock(newBlock.getHeader().getScryptHash(), txOutChanges));
        return newBlock;
    }
    ...
    protected StoredBlock addToBlockStore(StoredBlock storedPrev, Block block)
        ...
        blockStore.put(newBlock, new StoredUndoableBlock(newBlock.getHeader().getScryptHash(), block.transactions));
        return newBlock;
    }
    ...
    protected TransactionOutputChanges connectTransactions(int height, Block block)
        ...
        if (!params.passesCheckpoint(height, block.getScryptHash()))
            ...
            for (final Transaction tx : block.transactions) {
                ...
                if (!isCoinBase) {
                    ...
                    for (int index = 0; index < tx.getInputs().size(); index++) {
                        ...
                        StoredTransactionOutput prevOut = blockStore.getTransactionOutput(in.getOutpoint().getScryptHash(),
    ...
    protected synchronized TransactionOutputChanges connectTransactions(StoredBlock newBlock)
        ...
        checkState(lock.isHeldByCurrentThread());
        if (!params.passesCheckpoint(newBlock.getHeight(), newBlock.getHeader().getScryptHash()))
            throw new VerificationException("Block failed checkpoint lockin at " + newBlock.getHeight());
        
        blockStore.beginDatabaseBatchWrite();
        StoredUndoableBlock block = blockStore.getUndoBlock(newBlock.getHeader().getScryptHash());
        if (block == null) {
            // We're trying to re-org too deep and the data needed has been deleted.
            blockStore.abortDatabaseBatchWrite();
            throw new PrunedException(newBlock.getHeader().getScryptHash());
        }
    ...
    protected void disconnectTransactions(StoredBlock oldBlock) throws PrunedException, BlockStoreException {
        ...
        blockStore.beginDatabaseBatchWrite();
        try {
            StoredUndoableBlock undoBlock = blockStore.getUndoBlock(oldBlock.getHeader().getScryptHash());
            if (undoBlock == null) throw new PrunedException(oldBlock.getHeader().getScryptHash());
