...
import org.ohmcoinj.store.BlockStore;
 ...
 * {@link org.ohmcoinj.net.AbstractTimeoutHandler} and timeout is automatically disabled (using
 * {@link org.ohmcoinj.net.AbstractTimeoutHandler#setTimeoutEnabled(boolean)}) once the version
...
public class Peer extends PeerSocketHandler {
     ...
     * {@link org.ohmcoinj.net.NioClientManager#openConnection(java.net.SocketAddress, org.ohmcoinj.net.StreamParser)}
     * or
     * {@link org.ohmcoinj.net.NioClient#NioClient(java.net.SocketAddress, org.ohmcoinj.net.StreamParser, int)}.</p>
     *
    ...
    private void processVersionMessage(VersionMessage m) throws ProtocolException {
        ...
        // ohmcoinj is a client mode implementation. That means there's not much point in us talking to other client
    ...
    private void processBlock(Block m) {
        ...
        pendingBlockDownloads.remove(m.getScryptHash());
        try {
            ...
            if (blockChain.add(m)) {
                ...
                try {
                    if (downloadBlockBodies) {
                        final Block orphanRoot = checkNotNull(blockChain.getOrphanRoot(m.getScryptHash()));
                        blockChainDownloadLocked(orphanRoot.getScryptHash());
                    } else {
    ...
    private void endFilteredBlock(FilteredBlock m) {
        if (log.isDebugEnabled())
            log.debug("{}: Received broadcast filtered block {}", getAddress(), m.getScryptHash().toString());
        if (!vDownloadData) {
            log.debug("{}: Received block we did not ask for: {}", getAddress(), m.getScryptHash().toString());
            return;
        }
        if (blockChain == null) {
        ...
        pendingBlockDownloads.remove(m.getBlockHeader().getScryptHash());
        try {
            ...
            try {
                if (awaitingFreshFilter != null) {
                    log.info("Discarding block {} because we're still waiting for a fresh filter", m.getScryptHash());
                    ...
                    awaitingFreshFilter.add(m.getScryptHash());
                    ...
                    log.info("Bloom filter exhausted whilst processing block {}, discarding", m.getScryptHash());
    ...
    public void setDownloadParameters(long secondsSinceEpoch, boolean useFilteredBlocks) {
        ...
        try {
            ...
            this.useFilteredBlocks = false;//useFilteredBlocks;
        } finally {
            lock.unlock();
        }
    }
    ...
    private void blockChainDownloadLocked(Sha256Hash toHash) {
        ...
        Sha256Hash chainHeadHash = chainHead.getHeader().getScryptHash();
        ...
        for (int i = 100; cursor != null && i > 0; i--) {
            blockLocator.add(cursor.getHeader().getScryptHash());
            try {
        ...
        if (cursor != null)
            blockLocator.add(params.getGenesisBlock().getScryptHash());
     ...
     * Once the pong is received the value returned by {@link org.ohmcoinj.core.Peer#getLastPingTime()} is
     * updated.
    ...
    /**
     * Returns the elapsed time of the last ping/pong cycle. If {@link org.ohmcoinj.core.Peer#ping()} has never
    ...
    /**
     * Returns a moving average of the last N ping/pong cycles. If {@link org.ohmcoinj.core.Peer#ping()} has never
